...
def _gemini_generate_image_bytes(prompt: str) -> bytes:
    ...
    Falls back to Pollinations AI (free/keyless) and Google Gemini (paid/free tier) on failure.
    """
    # 1. Try Hugging Face
    ...
    if hf_key:
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"Hugging Face image attempt {attempt+1} failed: {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                
    # 2. Fallback to Pollinations AI (completely free and keyless)
    try:
        import urllib.parse
        ...
        print(f"Attempting Pollinations AI image generation fallback...")
        response = requests.get(pollinations_url, timeout=60)
        response.raise_for_status()
        if len(response.content) > 1000:
            return response.content
    except Exception as e:
        print(f"Pollinations AI fallback failed: {e}")
    ...
    if api_key:
        try:
            ...
            print(f"Gemini image fallback failed: {e}")

    raise RuntimeError("No image generation service succeeded (Hugging Face, Pollinations AI, or Gemini).")
